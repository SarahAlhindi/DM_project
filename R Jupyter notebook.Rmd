...
```{r}
caret::confusionMatrix(balanced_dataset$salary_in_usd,gainRatio10$finalModel$predictions)
...
caret::confusionMatrix(balanced_dataset$salary_in_usd,gainRatio5$finalModel$predictions)
...
caret::confusionMatrix(balanced_dataset$salary_in_usd,gainRatio3$finalModel$predictions)
...
so, we obtained the first number of clusters which equals 25, significantly this number is large, now let's try another method which is the elbow method
...
```{r}
# Plot the curve using fviz_nbclust
fviz_nbclust(dataset, kmeans, method = "wss") +
  geom_vline(xintercept = 4, linetype = 2)+
  labs(subtitle = "Elbow method")
```
so, we obtained the second number of clusters which equals TODO, now let's try another method which is the Silhouette method
