...
the tree has been divided based on the experiance level=3 which it is SE(Senior level)
then it divided to work year and experiance level=2 which it is MI(Mid level)
The decision tree iteratively chooses the best features to divide the dataset at each node
until it reaches the leaves, where it decides what category or label to assign.
