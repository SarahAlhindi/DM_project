...
### Find the outliers and remove them from the dataset:
by bloxPlot
now we show (salary_in_usd) attributes' outliers and remove them
we can see that there are many outliers with exceptionally high values
```{r}
...
now we show (remote_ratio) attributes' outliers and remove them
we can see there aren't outliers in remote_ratio
```{r}
...
now we show (work_year) attributes' outliers and remove them
we can see there aren't outliers in work_year
```{r}
