...
## Classificaation results

this table displays all results of the three classification algorithms with 3 different fold sizes
...
Based on this metrics, we can assess the performance of each K value:

-   Average Silhouette Width: The highest value is achieved at K = 2 (0.34), indicating that the clusters are relatively well-separated compared to the other K values.
...
-   Visualization: At K = 3 and K = 4, there is an overlap between the clusters, unlike K = 2, which provides two pure clusters.

Considering this metrics, we can conclude that K = 2 is the optimal k and performs comparatively better in terms of average silhouette width, BCubed precision, and BCubed recall. It indicates that the clustering with K = 2 leads to well-separated clusters without overlapping between clusters and with relatively good precision and recall. Which means the data in a cluster are close "similar" to each other and dissimilar to data in the other cluster "reflecting high intra-cluster similarity". And the reason for the overlapping and the low average silhouette width , precision, and recall results in K=3 and K=4 is could be duo to presence of outliers or sensitivity to Initial Centroid and high inter-cluster similarity.

## Findings

At the onset, our focus was on a dataset representing cyber security professionals' information to predict salary probabilities. The intent was to empower individuals with insights for negotiating better compensation and guiding career decisions in this field.
To ensure accurate and efficient results, we initiated several preprocessing techniques to refine the data. Employing tools like box plots and histograms, we visually analyzed the dataset to comprehend its nuances better. Through this exploration, we identified and subsequently removed anything that could potentially skew our analysis as outliers. Moreover, we conducted data transformations, normalizing and discretizing certain attributes. This step aimed to standardize attributes, granting them equal weightage, and simplifying data handling during subsequent mining tasks.
Therefore, we moved forward to implement both classification and clustering as part of our data mining process:
In the classification we used K-fold Cross-validation method as a dataset partitioning method, for each attribute selection method we used different K size 10,5 and 3. We evaluated each method's effectiveness using Accuracy, Precision, Sensitivity, Specificity metrics.
our findings led us to determine that among the methods tested, the gain ratio criterion paired with 10-fold cross-validation showcased the most effective performance for our dataset.

In the clustering process using k-means, we experimented with 3 different cluster numbers—2, 3, and 4—assessing their suitability through the silhouette and elbow methods. Opting for 2 clusters struck a balance between simplicity and complexity. Subsequently, we performed k-means clustering based on these numbers, evaluating the outcomes with various metrics. Our analysis highlighted that k=2 displayed superior performance, standing out for its distinct clusters without overlap. The data within each cluster exhibited significant similarity while being notably dissimilar from the other cluster, affirming k=2 as the optimal choice for this clustering scenario.

Ultimately, both models prove valuable in predicting cybersecurity salaries. The choice between classification and clustering hinges on specific objectives. 
Classification is advantageous for predicting salary ranges or categories based on known employee attributes, aiding individual compensation decisions. When it is essential to predict salary categories, employing the Gain ratio in classification proves beneficial.
Clustering aids in identifying natural employee groupings, revealing trends and common characteristics, facilitating market segmentation, and informing broader strategic decisions. Pursuing the goal of uncovering natural groupings in salary data, K=2 clustering stands out for its distinct clusters.

## Refrences

[1] J. Han, M. Kamber, and J. Pei, "Data Mining: Concepts and Techniques," 3rd ed., The Morgan Kaufmann Series in Data Management Systems. [2] Y. Zhao, "R and Data Mining: Examples and Case Studies," 1st ed. Academic Press, 2012. ISBN: 0123969638. [3] Y. Zhao, "R and Data Mining," RDataMining.com, Available: <https://www.rdatamining.com/>, Accessed on: November 23, 2023. [4] M. Hahsler, "discretize {arules}R Documentation: Convert a Continuous Variable into a Categorical Variable," R Project, Available: <https://search.r-project.org/CRAN/refmans/arules/html/discretize.html>, Accessed on: November 23, 2023.
