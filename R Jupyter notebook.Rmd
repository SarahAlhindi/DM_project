...
so, this is the model that we have chosen as our classification model
...
At the onset, our focus was on a dataset representing cyber security professionals' information to predict salary probabilities. The intent was to empower individuals with insights for negotiating better compensation and guiding career decisions in this field. To ensure accurate and efficient results, we initiated several preprocessing techniques to refine the data. Employing tools like box plots and histograms, we visually analyzed the dataset to comprehend its nuances better. Through this exploration, we identified and subsequently removed anything that could potentially skew our analysis as outliers. Moreover, we conducted data transformations, normalizing and discretizing certain attributes. This step aimed to standardize attributes, granting them equal weightage, and simplifying data handling during subsequent mining tasks. Therefore, we moved forward to implement both classification and clustering as part of our data mining process: In the classification we used K-fold Cross-validation method as a dataset partitioning method, for each attribute selection method we used different K size 10,5 and 3. We evaluated each method's effectiveness using Accuracy, Precision, Sensitivity, Specificity metrics. our findings led us to determine that among the methods tested, the gain ratio criterion paired with 10-fold cross-validation showcased the most effective performance for our dataset.
