...
#### k-means clustering 1 and visualization the clustering
...
precision <- bcubed_precision(kmeans.result, classLabel)
print(precision)

#calculate recall

recall <- bcubed_recall(kmeans.result, classLabel)
print(recall)
```



#### k-means clustering 2 and visualization the clustering
...
#### k-means clustering 3 and visualization the clustering
...
```{r}
bcubed_precision <- function(kmeans.result, classLabel) {
  n <- length(kmeans.result)
  precision <- 0
  
  kmeans.result <- as.character(kmeans.result)
  for (i in 1:n) {
    cluster_size <- sum(kmeans.result == kmeans.result[i])
    common_elements <- sum(classLabel == classLabel[i] & kmeans.result == kmeans.result[i])
    
    if (cluster_size > 1) {
      precision <- precision + common_elements / cluster_size
    }
  }
...
```{r}
bcubed_recall <- function(kmeans.result, classLabel) {
  n <- length(kmeans.result)
  recall <- 0
  
  for (i in 1:n) {
    cluster_size <- sum(classLabel == classLabel[i])
    common_elements <- sum(classLabel == classLabel[i] & kmeans.result == kmeans.result[i])
    
    if (cluster_size > 1) {
      recall <- recall + common_elements / cluster_size
    }
  }
